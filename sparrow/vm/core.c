...
static bool primObjectNot(VM* vm  , Value* args) {
...
static bool primObjectNotEqual(VM* vm  , Value* args) {
   Value boolValue = BOOL_TO_VALUE(!valueIsEqual(args[0], args[1]));
   RET_VALUE(boolValue);
}
...
static bool primObjectToString(VM* vm  , Value* args) {
...
//bool值取反
static bool primBoolNot(VM* vm  , Value* args) {
...
static bool primThreadCurrent(VM* vm, Value* args  ) {
   RET_OBJ(vm->curThread);
}

//Thread.suspend():挂起线程,退出解析器
static bool primThreadSuspend(VM* vm, Value* args  ) {
   //目前suspend操作只会退出虚拟机,
...
static bool primThreadYieldWithoutArg(VM* vm, Value* args  ) {
   ObjThread* curThread = vm->curThread;
   vm->curThread = curThread->caller;   //使cpu控制权回到主调方
...
static bool primThreadIsDone(VM* vm  , Value* args) {
   //获取.isDone的调用者
...
//null取非
static bool primNullNot(VM* vm  , Value* args  ) {
   RET_VALUE(BOOL_TO_VALUE(true));
}

//null的字符串化
static bool primNullToString(VM* vm, Value* args  ) {
...
//返回圆周率
static bool primNumPi(VM* vm  , Value* args  ) {
...
//数字取反
static bool primNumBitNot(VM* vm  , Value* args) {
...
//返回小数部分
static bool primNumFraction(VM* vm  , Value* args) {
   double dummyInteger;
   RET_NUM(modf(VALUE_TO_NUM(args[0]), &dummyInteger));
}

//判断数字是否无穷大,不区分正负无穷大 
static bool primNumIsInfinity(VM* vm  , Value* args) {
   RET_BOOL(isinf(VALUE_TO_NUM(args[0])));  
}

//判断是否为数字
static bool primNumIsInteger(VM* vm  , Value* args) {
...
//判断数字是否为nan
static bool primNumIsNan(VM* vm  , Value* args) {
...
//取数字的整数部分
static bool primNumTruncate(VM* vm  , Value* args) {
   double integer;
...
static bool primStringByteAt(VM* vm  , Value* args) {
   ObjString* objString = VALUE_TO_OBJSTR(args[0]);
   uint32_t index = validateIndex(vm, args[1], objString->value.length);
   if (index == UINT32_MAX) {
      return false; 
...
static bool primStringByteCount(VM* vm  , Value* args) {
   RET_NUM(VALUE_TO_OBJSTR(args[0])->value.length);
}

//objString.codePointAt_(_):返回指定的CodePoint
static bool primStringCodePointAt(VM* vm  , Value* args) {
   ObjString* objString = VALUE_TO_OBJSTR(args[0]);
   uint32_t index = validateIndex(vm, args[1], objString->value.length);
   if (index == UINT32_MAX) {
      return false; 
...
static bool primStringToString(VM* vm  , Value* args) {
   RET_VALUE(args[0]);
}

//objList.new():创建1个新的liist
static bool primListNew(VM* vm, Value* args  ) {
...
static bool primListCount(VM* vm  , Value* args) {
...
//objMap.new():创建map对象
static bool primMapNew(VM* vm, Value* args  ) {
...
static bool primMapCount(VM* vm  , Value* args) {
...
//因此直接返回迭代器就是range的值
static bool primRangeIteratorValue(VM* vm  , Value* args) {
   ObjRange* objRange = VALUE_TO_OBJRANGE(args[0]);
...
static bool primSystemClock(VM* vm  , Value* args  ) {
...
static bool primSystemWriteString(VM* vm  , Value* args) {
   ObjString* objString = VALUE_TO_OBJSTR(args[1]);
