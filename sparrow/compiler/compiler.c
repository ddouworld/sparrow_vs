...
//为单运算符方法创建签名
static void unaryMethodSignature( CompileUnit* cu  , Signature* sign  ) {
    //名称部分在调用前已经完成,只修改类型
...
//'||'.led()
static void logicOr(CompileUnit * cu, bool canAssign  ) {
...
//'&&'.led()
static void logicAnd(CompileUnit * cu, bool canAssign  ) {
    //此时栈顶是条件表达式的结果,即"&&"的左操作数
...
//"? :".led()
static void condition(CompileUnit * cu, bool canAssign  ) {
...
//"this".nud()
static void this(CompileUnit * cu, bool canAssign  ) {
...
//数字和字符串.nud() 编译字面量
static void literal(CompileUnit * cu, bool canAssign  ) {
...
//编译圆括号
static void parentheses(CompileUnit * cu, bool canAssign  ) {
...
static void listLiteral(CompileUnit * cu, bool canAssign  ) {
...
//map对象字面量
static void mapLiteral(CompileUnit * cu, bool canAssign  ) {
...
//内嵌表达式.nud()
static void stringInterpolation(CompileUnit * cu, bool canAssign  ) {
    // "a %(b+c) d %(e) f " 
    // 会按照以下形式编译
    // ["a ", b+c, " d ", e, "f "].join() 
...
//编译bool
static void boolean(CompileUnit * cu, bool canAssign  ) {
    //true和false的nud方法
...
static void null(CompileUnit * cu, bool canAssign  ) {
    writeOpCode(cu, OPCODE_PUSH_NULL);
}
...
//占位用的
#define  _RULE {NULL, BP_NONE, NULL, NULL, NULL}
